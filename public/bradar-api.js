        ...
        try {
          ...
          api('/api/analyze', { method: 'POST', body: JSON.stringify(body) }).then(function (plan) {
            ...
            if (plan && Array.isArray(plan.channels) && plan.channels.length) {
              ...
              S._insight = plan.insight || null;                    // AI reasoning: brand → buyer → where
              S._salesKit = (plan.outreach || (plan.dealTips && plan.dealTips.length))
                ? { outreach: plan.outreach || '', dealTips: Array.isArray(plan.dealTips) ? plan.dealTips : [] }
                : null;                                             // paid layer: ready outreach + deal tips
              S._noData = false;
