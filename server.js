  ...
  try {
    ...
    if (p === '/api/telegram/webhook' && req.method === 'POST') {
      ...
      if (upd.pre_checkout_query) {
        ...
        else await tg('sendMessage', { chat_id: upd.message.chat.id, text, reply_markup: kb });
    ...
    if (p.startsWith('/api/')) {
      ...
      if (p === '/api/config' && req.method === 'GET') {
        ...
        return send(res, 200, {
          ...
          botUsername: (process.env.BOT_USERNAME || 'BRADARP_BOT').replace(/^@/, ''),   // for share deep links
